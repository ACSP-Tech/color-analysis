...
"""
async def extract_and_clean_colors(html_content):
    tbody_content = re.search(r'<tbody>(.*?)</tbody>', html_content, re.DOTALL)
    # # if not tbody_content:
    # #     return []
    # 2. Extract the color strings from the second <td> in each row
    raw_color_strings = re.findall(r'<tr>\s*<td>.*?</td>\s*<td>(.*?)</td>\s*</tr>', 
                                tbody_content.group(1), re.IGNORECASE | re.DOTALL)
    all_colors = []
    for day_colors in raw_color_strings:
        # Split by comma, strip whitespace, and convert to uppercase for consistency
        colors = [color.strip().upper() for color in day_colors.split(',')]
        all_colors.extend(colors)
    #3. Data Cleaning: Correct obvious typos
    cleaned_colors = [
        'BLUE' if color == 'BLEW' else 'ASH' if color == 'ARSH' else color for color in all_colors]
